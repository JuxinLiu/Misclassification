# Y subject to nondifferential errors and X subject to differential errors
...
# function that returns D values that produce max and min \delta with para in order of SNX1, SNX0, SPX1, SPX0, SNY, SPY
DD <- function(para) {
  eps <- 0.001
  SNX1 <- para[1]
  SNX0 <- para[2]
  ...
  SNY <- para[5]
  SPY <- para[6]
  L1 <- max(-SNY * SNX1, -(1 - SNY) * (1 - SNX1))
  U1 <- min((1 - SNY) * SNX1, SNY * (1 - SNX1))
  L2 <- max(-SNY * SPX1, -(1 - SNY) * (1 - SPX1))
  U2 <- min((1 - SNY) * SPX1, SNY * (1 - SPX1))
  L3 <- max(-SPY * SNX0, -(1 - SPY) * (1 - SNX0))
  U3 <- min((1 - SPY) * SNX0, SPY * (1 - SNX0))
  L4 <- max(-SPY * SPX0, -(1 - SPY) * (1 - SPX0))
  U4 <- min((1 - SPY) * SPX0, SPY * (1 - SPX0))
  obj <- function(D) {
...
#### columns in the order of SNX1, SNX0, SPX1, SPX0, SNY, SPY
...
for (i in 1:4) {
  ...
  for (j in 1:4) {
    ...
    C0[i, j] <- (SNY^ystar * (1 - SNY)^(1 - ystar))^y * ((1 - SPY)^ystar * SPY^(1 - ystar))^(1 - y) * (SNX[2 - y]^xstar * (1 - SNX[2 - y])^(1 - xstar))^x *
      ((1 - SPX[2 - y])^xstar * SPX[2 - y]^(1 - xstar))^(1 - x)
...
D0 <- rep(NA, 1)
D0[1] <- all$count[all$ystar == 1 & all$xstar == 1 & all$y == 1 & all$x == 1]/sum(all$count[all$y == 1 & all$x == 1]) - SNY_v * SNX1_v
D0[2] <- all$count[all$ystar == 1 & all$xstar == 0 & all$y == 1 & all$x == 0]/sum(all$count[all$y == 1 & all$x == 0]) - SNY_v * SPX1_v
D0[3] <- all$count[all$ystar == 0 & all$xstar == 1 & all$y == 0 & all$x == 1]/sum(all$count[all$y == 0 & all$x == 1]) - SPY_v * SNX0_v
D0[4] <- all$count[all$ystar == 0 & all$xstar == 0 & all$y == 0 & all$x == 0]/sum(all$count[all$y == 0 & all$x == 0]) - SPY_v * SPX0_v
...
############nondifferential Y, differential X, and dependent model
...
for (i in 1:4) {
    ...
    for (j in 1:4) {
        ...
        C[i, j] <- (SNY^ystar[i, j] * (1 - SNY)^(1 - ystar[i, j]))^y[i, j] * ((1 - SPY)^ystar[i, j] * SPY^(1 - ystar[i, j]))^(1 - y[i, j]) * (SNX[2 -
            y[i, j]]^xstar[i, j] * (1 - SNX[2 - y[i, j]])^(1 - xstar[i, j]))^x[i, j] * ((1 - SPX[2 - y[i, j]])^xstar[i, j] * SPX[2 - y[i, j]]^(1 - xstar[i,
...
L[1] <- max(-SNY * SNX1, -(1 - SNY) * (1 - SNX1))
U[1] <- min((1 - SNY) * SNX1, SNY * (1 - SNX1))
L[2] <- max(-SNY * SPX1, -(1 - SNY) * (1 - SPX1))
U[2] <- min((1 - SNY) * SPX1, SNY * (1 - SPX1))
L[3] <- max(-SPY * SNX0, -(1 - SPY) * (1 - SNX0))
U[3] <- min((1 - SPY) * SNX0, SPY * (1 - SNX0))
L[4] <- max(-SPY * SPX0, -(1 - SPY) * (1 - SPX0))
U[4] <- min((1 - SPY) * SPX0, SPY * (1 - SPX0))
...
inits1 <- list(SNX0 = SNX0_v, SPX0 = SPX0_v, SNX1 = SNX1_v, SPX1 = SPX1_v, SNY = SNY_v, SPY = SPY_v, p1 = px0, Dt = rep(0.8, 4), beta0 = beta0_ini, beta1 = beta1_ini,
               .RNG.name = "base::Super-Duper", .RNG.seed = 1)
inits2 <- list(SNX0 = SNX0_v, SPX0 = SPX0_v, SNX1 = SNX1_v, SPX1 = SPX1_v, SNY = SNY_v, SPY = SPY_v, p1 = px0, Dt = rep(0.5, 4), beta0 = beta0_ini, beta1 = beta1_ini,
               .RNG.name = "base::Wichmann-Hill", .RNG.seed = 2)

opt_dd <- run.jags(model = ddmodely, monitor = monitor, data = data, n.chains = 2, adapt = 5000, sample = 10000, burnin = 5000, method = "rjags", inits = list(inits1,
                                                                                                                                                               inits2))
...
inits1 <- list(SNX0 = SNX0_v, SPX0 = SPX0_v, SNX1 = SNX1_v, SPX1 = SPX1_v, SNY = SNY_v, SPY = SPY_v, p1 = px0, beta0 = beta0_ini, beta1 = beta1_ini, .RNG.name = "base::Super-Duper",
               .RNG.seed = 1)
inits2 <- list(SNX0 = SNX0_v, SPX0 = SPX0_v, SNX1 = SNX1_v, SPX1 = SPX1_v, SNY = SNY_v, SPY = SPY_v, p1 = 0.2, beta0 = beta0_ini, beta1 = beta1_ini, .RNG.name = "base::Wichmann-Hill",
               .RNG.seed = 2)

opt_di <- run.jags(model = dimodel, monitor = monitor, data = data, n.chains = 2, adapt = 5000, sample = 10000, burnin = 5000, method = "rjags", inits = list(inits1,
                                                                                                                                                              inits2))
